  ...
  <div class="view-container">
    ...
    <el-form
      ...
      <el-form-item prop="area">
        <el-select v-model="searchForm.cityCode" clearable placeholder="请选择所属城市">
          <el-option
      ...
      <el-form-item prop="shelfStatus">
        <el-select v-model="searchForm.frontStatus" clearable placeholder="请选择订单状态">
          <el-option
    ...
    <el-table
      ...
      <el-table-column prop="code" label="销售订单编号" min-width="120" />
      ...
      <el-table-column prop="oilDepotCityName" label="采购城市" min-width="120" />
      <el-table-column prop="oilDepotName" label="货源类型" min-width="120" />
      <el-table-column prop="marketPrice" label="商品类型" min-width="120">
        <template #default="scope">
          <div>{{ `${scope.row.marketPrice}元/${scope.row.unitName}` }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="salePrice" label="提油方式" min-width="120">
        <template #default="scope">
          <div>{{ `${scope.row.salePrice}元/${scope.row.unitName}` }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="totalQuantity" label="采购数量" min-width="120">
        <template #default="scope">
          <div>{{ scope.row.totalQuantity + scope.row.unitName }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="minQuantity" label="油品单价" min-width="100">
        <template #default="scope">
          <div>{{ scope.row.minQuantity + scope.row.unitName }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="supportLockPriceName" label="订单总金额(元)" width="80" />
      <el-table-column prop="promotionTypeName" label="应收总金额(元)" width="80" />
      <el-table-column prop="statusName" label="实收金额(元)" width="80" />
      <el-table-column prop="statusName" label="订单差额(元)" width="80" />
      <el-table-column prop="statusName" label="支付状态" width="80" />
